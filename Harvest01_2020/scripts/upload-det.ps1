<#
.SYNOPSIS
Uploads data entry templates from Harvest01_2020 folder to Azure Blob Storage then moves the uploaded file to local folder for archival
.PARAMETER Directory
...
# Get all xlsm files in Harvest01_2020, loop through and upload/archive
Get-ChildItem "$($Directory)\Harvest01_2020" -Filter *.xlsm |
ForEach-Object {
    # Get file info
    $filePath = $_.FullName
    ...
    # Prepare for upload
    $uri = "https://cafltardatastream.blob.core.windows.net/cafsampleprocessing/Harvest01_2020/$($name)$($sas)"
